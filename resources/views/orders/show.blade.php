...
<x-app-layout>
    ...
    @php
        ...
        $hasAdvisorSnapshot = $advisorSnapshot !== null;
        $displayAdvisor = $advisorSnapshot ?? $currentOrderAdvisor;
        $displayAdvisorName = $hasAdvisorSnapshot
            ? $advisorSnapshot?->validName()
            : $currentOrderAdvisor?->validName();
        $advisorWhatsappUrl = $hasAdvisorSnapshot
            ? $advisorSnapshot?->whatsappUrl($advisorWhatsappMessage)
            : ($currentOrderAdvisor?->whatsappUrl($advisorWhatsappMessage) ?? (($supportWhatsappNumber ?? null) !== null ? 'https://wa.me/'.$supportWhatsappNumber.'?text='.rawurlencode($advisorWhatsappMessage) : null));
    @endphp
    ...
    <section class="grid gap-4 xl:grid-cols-[1.7fr_1fr]">
        ...
        <div class="space-y-4">
            ...
            @if($displayAdvisorName)
                <x-ui.card class="p-5">
                    <h2 class="card-title">{{ $hasAdvisorSnapshot ? 'Asesor comercial' : 'Contacto comercial actual' }}</h2>
                    <p class="mt-2 text-sm font-semibold text-slate-900">{{ $displayAdvisorName }}</p>
                </x-ui.card>
            @endif
        </div>
    </section>
