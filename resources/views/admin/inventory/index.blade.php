...
<x-app-layout>
    <x-slot name="header">
        <section class="admin-exec-hero">
            <div class="admin-exec-hero-main">
                ...
                <p class="admin-exec-hero-subtitle">Sincroniza productos y niveles de stock desde InvenTree API.</p>
            ...
            <div class="mt-4 grid gap-4 md:grid-cols-2">
                <div class="rounded-xl border border-slate-200 p-4">
                    ...
                    <dl class="mt-2 space-y-1 text-sm">
                        <div class="flex justify-between"><dt class="text-slate-500">Total en InvenTree:</dt><dd class="font-medium">{{ $syncResults['products']['total'] ?? 0 }}</dd></div>
                        <div class="flex justify-between"><dt class="text-slate-500">Creados:</dt><dd class="font-medium text-green-600">{{ $syncResults['products']['created'] ?? 0 }}</dd></div>
                        <div class="flex justify-between"><dt class="text-slate-500">Actualizados:</dt><dd class="font-medium text-amber-600">{{ $syncResults['products']['updated'] ?? 0 }}</dd></div>
                        <div class="flex justify-between"><dt class="text-slate-500">Sin cambios:</dt><dd class="font-medium text-slate-400">{{ $syncResults['products']['skipped'] ?? 0 }}</dd></div>
                        <div class="flex justify-between"><dt class="text-slate-500">Errores:</dt><dd class="font-medium text-red-600">{{ $syncResults['products']['errors'] ?? 0 }}</dd></div>
                    </dl>
                </div>
                <div class="rounded-xl border border-slate-200 p-4">
    ...
    <div class="grid gap-4 md:grid-cols-2">
        <x-ui.card class="p-5">
            <h2 class="card-title">Sincronizar productos</h2>
            <p class="mt-1 text-sm text-slate-500">Trae todos los productos activos desde InvenTree y los crea o actualiza en el portal usando el SKU como identificador.</p>
        ...
        <x-ui.card class="p-5 md:col-span-2">
            ...
            <p class="mt-1 text-sm text-slate-500">Ejecuta la sincronización completa: productos y stock en una sola operación.</p>
            <form action="{{ route('admin.inventory.sync') }}" method="POST" class="mt-4">
                @csrf
