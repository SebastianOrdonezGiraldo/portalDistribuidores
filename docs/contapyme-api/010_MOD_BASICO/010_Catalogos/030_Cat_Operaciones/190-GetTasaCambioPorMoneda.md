...
> Fuente original: https://www.contapyme.com/api/010_MOD_BASICO/010_Catalogos/030_Cat_Operaciones/190-GetTasaCambioPorMoneda.html

Â¿CÃ³mo verificar la tasa de cambio por moneda?

GetTasaCambioPorMoneda (datajson, controlkey, iapp, random) : json
...
Esta función es la encargada de calcular y retornar la tasa de cambio actual para la moneda que se envía en el llamado de la función.

#### Resultado

Retorna un Json con la tasa de cambio actual para una moneda dada.
...
| --- | --- | --- | --- |
| dataJSON | JSON | Json que contiene en su interior la siguiente información:  **imoneda:** Código de la moneda para cual se desea obtener la tasa de cambio. (requerido) **fsoport:** Fecha para la cual se obtendrá la tasa de cambio. (requerido) | { "imoneda": "10", "fsoport": "04/16/2013" } |
...
var URLFuncion = '/datasnap/rest/TCatOperaciones/"GetTasaCambioPorMoneda"/';
...
Ver documentaciÃ³n de la peticiÃ³n por [GET.](PDFS/190-GetTasaCambioPorMoneda.pdf)
...
| respuesta | JSON | Json que contiene en su interior el objeto “datos” que se describe a continuación:  **mtasacambio (varchar):** Valor de la tasa de cambio para la moneda enviada en la petición. |
