...
> Fuente original: https://www.contapyme.com/api/010_MOD_BASICO/010_Catalogos/030_Cat_Operaciones/040-GetAutoLista.html

Â¿CÃ³mo verificar la lista automatica?

GetAutoLista (datajson, controlkey, iapp, random) : json
...
Esta función se debe llamar en cada campo que presente un listado de datos; para saber cuáles campos son de tipo auto-lista, la función “GetConfig” de cada operación, retornará en estos campos el parámetro “itdlista” en 1.

#### Resultado

Retorna un Json con los datos almacenados en un campo.
...
| --- | --- | --- | --- |
| dataJSON | JSON | Json que contiene en su interior la siguiente estructura.  **campo:** Nombre del campo de tipo auto-lista del cual se desea obtener el listado de valores que tiene registrados.  Los nombres de los campos se obtienen con la función GetConfig de cada operación que se encuentre en la clase TCatOperaciones. (requerido).  **seccion:** Nombre de la sección en la que se encuentra el campo.  Los nombres de las secciones se obtienen con la función GetConfig de cada operación que se encuentre en la clase TCatOperaciones.  (requerido).  **filtro**: Filtro que se debe aplicar para obtener el listado, este parámetro se debe enviar cuando en la función GetConfig de cada operación, el parámetro “ncampofiltro” retorne algún valor. (opcional).  **itdoper:** Identificador del tipo de operación para la cual se está obteniendo el listado de valores. Los tipos de operación se pueden consultar en el documento: “TiposDeOperaciones” de la zona de “Documentación de apoyo”. (requerido). | { "campo": "svaloradic1", "seccion": "encabezado", "filtro": "", "itdoper": "ORD1" } |
...
var URLFuncion = '/datasnap/rest/TCatOperaciones/"GetAutoLista"/';
...
Ver documentaciÃ³n de la peticiÃ³n por [GET.](PDFS/040-GetAutoLista.pdf)
...
| respuesta | JSON | Json que contiene en su interior el arreglo “datos” que se describe a continuación:  **datos (arreglo)**: Contiene los valores para el campo que fue solicitado en la petición. |
