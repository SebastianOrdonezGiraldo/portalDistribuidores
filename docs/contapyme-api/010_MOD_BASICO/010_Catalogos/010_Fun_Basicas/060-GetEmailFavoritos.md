...
> Fuente original: https://www.contapyme.com/api/010_MOD_BASICO/010_Catalogos/010_Fun_Basicas/060-GetEmailFavoritos.html

Â¿CÃ³mo obtener el email favorito?

GetEmailFavoritos (datajson, controlkey, iapp, random): json
...
Esta función es la encargada de retornar el listado de los correos electrónicos que tiene asociados un usuario del sistema, estos correos se conocen como los email’s favoritos del usuario y es posible configurar uno o más emails favoritos.

#### Resultado

Retorna un json con el listado correos electrÃ³nicos que tiene asignados un usuario del sistema.
...
| --- | --- | --- | --- |
| dataJSON | JSON | No se requiere ningún dato, pues el listado de emails se obtiene de acuerdo al controlkey enviado en la petición. | { } |
...
var URLFuncion = '/datasnap/rest/TBasicoGeneral/"GetEmailFavoritos"/';
...
Ver documentaciÃ³n de la peticiÃ³n por [GET.](PDFS/060-GetEmailFavoritos.pdf)
...
| respuesta | JSON | Json que contiene en su interior el arreglo de objetos “datos” el cual se describe a continuación:  **nombre (varchar)**: Nombre que se asigna para identificar el email favorito. **Email (varchar)**: Dirección de correo electrónico que el usuario registra como favorita.  . |
