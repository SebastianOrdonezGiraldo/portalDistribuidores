...
> Fuente original: https://www.contapyme.com/api/010_MOD_BASICO/010_Catalogos/020_Cat_Terceros/070-GetExisteTercero.html

Â¿CÃ³mo validar si un tercero existe en ContaPyme/AgroWin?

GetExisteTercero (datajson, controlkey, iapp, random) : json
...
Esta función es la encargada de validar si un tercero ya existe en la base de datos, recibe el código del tercero a validar y retorna True si el tercero existe o False si el tercero no existe en la base de datos.

#### Resultado

Retorna un Json con la confirmaciÃ³n de existencia del tercero
...
| --- | --- | --- | --- |
| dataJSON | JSON | Json que contiene en su interior la siguiente estructura.  **init:** Código del tercero del cual se va a validar su existencia. (requerido) | { "init": "810000630" } |
...
var URLFuncion = '/datasnap/rest/TCatTerceros/"GetExisteTercero"/';
...
Ver documentaciÃ³n de la peticiÃ³n por [GET.](PDFS/070-GetExisteTercero.pdf)
...
| respuesta | JSON | Json que contiene en su interior el objeto “datos” que se describe a continuación. **existe (varchar):** Contiene true si el tercero existe y false si no existe. |
