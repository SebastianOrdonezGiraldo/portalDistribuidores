...
> Fuente original: https://www.contapyme.com/api/010_MOD_BASICO/010_Catalogos/020_Cat_Terceros/080-GetValidarNuevoIdTercero.html

Â¿CÃ³mo validar el nuevo ID de un tercero en ContaPyme/AgroWin?

GetValidarNuevoIdTercero (datajson, controlkey, iapp, random) : json
...
Esta función es la encargada de validar si el código (init) de un nuevo tercero y su dígito de verificación son correctos, es decir, valida que no contengan caracteres especiales. También valida que el dígito de verificación sea el correspondiente para el código recibido.

#### Resultado

Retorna un Json con la confirmaciÃ³n de la validez del cÃ³digo de un nuevo tercero.
...
| --- | --- | --- | --- |
| dataJSON | JSON | Json que contiene en su interior la siguiente estructura.  **init:** Identificador único del tercero. (requerido) **digitoverif:** Dígito de verificación a validar con el init. **bdigitoverificacion:** Indica si se debe validar el dígito de verificación enviado en digitoverif. Si está en “T” validará que el dígito de verificación sea el correspondiente para el init recibido. | { "init": "810000630", "digitoverif": "8", "bdigitoverificacion": "t" } |
...
var URLFuncion = '/datasnap/rest/TCatTerceros/"GetValidarNuevoIdTercero"/';
...
Ver documentaciÃ³n de la peticiÃ³n por [GET.](PDFS/080-GetValidarNuevoIdTercero.pdf)
...
| respuesta | JSON | Json que contiene en su interior el objeto “datos” que se describe a continuación.  **initvalido (varchar):** contiene true cuando el código del nuevo tercero es válido y false cuando no lo es.  **dvvalido (varchar)**: contiene true cuando el dígito de verificación es correcto y es el correspondiente para el init ingresado, y contiene false cuando no lo es. |
