...
> Fuente original: https://www.contapyme.com/api/010_MOD_BASICO/010_Catalogos/020_Cat_Terceros/100-DoRecodificarTercero.html

Â¿CÃ³mo recodificar un tercero?

DoRecodificarTercero (datajson, controlkey, iapp, random) : json
...
Esta función es la encargada de recodificar un tercero, es decir, cambia el código de identificación del tercero tanto en el catálogo de terceros como en las operaciones en las que esté relacionado.

#### Resultado

Retorna un Json con la confirmaciÃ³n de la recodificaciÃ³n de los datos del tercero.
...
| --- | --- | --- | --- |
| dataJSON | JSON | Json que contiene en su interior la siguiente estructura.  **init:** Código actual del tercero que se quiere recodificar. (requerido) **newinit:** Nuevo código que se le asignará al tercero. (requerido) | { "init": "10538777", "newinit": "10538475" } |
...
var URLFuncion = '/datasnap/rest/TCatTerceros/"DoRecodificarTercero"/';
...
Ver documentaciÃ³n de la peticiÃ³n por [GET.](PDFS/100-DoRecodificarTercero.pdf)
...
| respuesta | JSON | Json que contiene en su interior el objeto “datos” que se describe a continuación:  **recodificar (varchar)**: contiene true cuando el tercero se recodifica satisfactoriamente y false cuando no se pudo recodificar. . |
...
- 210: El nuevo init no es vÃ¡lido.
