...
> Fuente original: https://www.contapyme.com/api/010_MOD_BASICO/010_Catalogos/020_Cat_Terceros/160-GetOperacionesPorTercero.html

Â¿CÃ³mo obtener las operaciones de un tercero?

GetOperacionesPorTercero (datajson, controlkey, iapp, random) : json
...
Retorna un Json con la cantidad de operaciones en las que estÃ¡ relacionado un tercero
...
| --- | --- | --- | --- |
| dataJSON | JSON | Json que contiene en su interior la siguiente estructura.  **init**: Código del tercero del cual se desea obtener la cantidad de operaciones en las que está relacionado, agrupando por tipo de documento. **fsoport**: Fecha hasta la cual se calculará la cantidad de operaciones. | { "init": "1053799", "fsoport": "03-02-2014" } |
...
var URLFuncion = '/datasnap/rest/TCatTerceros/"GetOperacionesPorTercero"/';
...
Ver documentaciÃ³n de la peticiÃ³n por [GET.](PDFS/160-GetOperacionesPorTercero.pdf)
...
| respuesta | JSON | Json que contiene en su interior el arreglo de objetos “datos” que se describe a continuación:  **itdsop (varchar)**: Código del tipo de documento de soporte **ntdsop (varchar):** Nombre del tipo de documento de soporte **qoprs (integer):** cantidad de operaciones existentes por tipo de documento de soporte . |
