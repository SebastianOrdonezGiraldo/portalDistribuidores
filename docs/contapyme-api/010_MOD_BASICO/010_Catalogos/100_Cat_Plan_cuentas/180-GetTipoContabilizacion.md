...
> Fuente original: https://www.contapyme.com/api/010_MOD_BASICO/010_Catalogos/100_Cat_Plan_cuentas/180-GetTipoContabilizacion.html

Â¿CÃ³mo verificar el tipo de contabilizaciÃ³n?

GetTipoContabilizaciÃ³n (datajson, controlkey, iapp, random) : json
...
Esta función es la encargada de retornar el tipo de contabilización que tiene configurado el sistema a una fecha determinada. La contabilización puede ser solo LOCAL, solo NIIF o Ambas.

#### Resultado

Retorna un Json con el tipo de contabilizaciÃ³n que tiene definido el sistema.
...
| --- | --- | --- | --- |
| dataJSON | JSON | Json que contiene en su interior la siguiente estructura:  **fsoport (varchar)**: fecha a la cual se desea consultar el tipo de contabilización configurado en el sistema. | { "fsoport": "09/25/2014" } |
...
var URLFuncion = '/datasnap/rest/TCatPlanCuentas/"GetTipocontabilizacion"/';
...
Ver documentaciÃ³n de la peticiÃ³n por [GET.](PDFS/180-GetTipoContabilizacion.pdf)
...
| respuesta | JSON | Json que contiene en su interior el objeto “datos” que se describe a continuación:  **blocal (varchar):** Contiene True cuando el tipo de contabilización es sólo Local.  **bniif** (varc**h**ar): Contiene True cuando el tipo de contabilización es sólo NIIF.  Cuando el tipo de contabilización es local y NIIF al mismo tiempo, los dos parámetros descritos anteriormente irán en True. |
