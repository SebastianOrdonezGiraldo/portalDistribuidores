...
> Fuente original: https://www.contapyme.com/api/010_MOD_BASICO/010_Catalogos/100_Cat_Plan_cuentas/080-GetValidarNuevoIdCuenta.html

Â¿CÃ³mo validar una cuenta por su nuevo ID?

GetValidarNuevoIdCuenta (datajson, controlkey, iapp, random) : Json
...
Esta función es la encargada de validar si el código (icuenta) de una nueva cuenta es correcto, es decir, valida que no contenga caracteres especiales.

#### Resultado

Retorna un Json con la confirmaciÃ³n de la validez del cÃ³digo de una nueva cuenta.
...
| --- | --- | --- | --- |
| dataJSON | JSON | Json que contiene en su interior la siguiente información:  **icuenta**: Identificador de la cuenta a validar | { "icuenta": "110526" } |
...
var URLFuncion = '/datasnap/rest/TCatPlanCuentas/"GetValidarNuevoIdCuenta"/';
...
Ver documentaciÃ³n de la peticiÃ³n por [GET.](PDFS/080-GetValidarNuevoIdCuenta.pdf)
...
| respuesta | JSON | Json que contiene en su interior el objeto “datos” que se describe a continuación.  **icuentavalido (varchar)**: contiene true cuando el código de la nueva cuenta es válido y false cuando no lo es. |
