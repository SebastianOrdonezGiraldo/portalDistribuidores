...
> Fuente original: https://www.contapyme.com/api/010_MOD_BASICO/010_Catalogos/100_Cat_Plan_cuentas/070-GetExisteCuenta.html

Â¿CÃ³mo verificar una cuenta existente?

GetExisteCuenta (datajson, controlkey, iapp, random) : json
...
Esta función es la encargada de validar si una cuenta ya existe en la base de datos, recibe el código de la cuenta a validar y retorna True si la cuenta existe o False si la cuenta no existe en la base de datos.

#### Resultado

Retorna un Json con la confirmaciÃ³n de existencia de una cuenta
...
| --- | --- | --- | --- |
| dataJSON | JSON | Json que contiene en su interior la siguiente estructura.  **icuenta:** Código de la cuenta de la cual se va a validar su existencia. (requerido) | { "icuenta": "110520" } |
...
var URLFuncion = '/datasnap/rest/TCatPlanCuentas/"GetExisteCuenta"/';
...
Ver documentaciÃ³n de la peticiÃ³n por [GET.](PDFS/070-GetExisteCuenta.pdf)
...
| respuesta | JSON | Json que contiene en su interior el objeto “datos” que se describe a continuación. **existe (varchar):** Contiene true si la cuenta existe y false si no existe. |
