...
> Fuente original: https://www.contapyme.com/api/040_INVENTARIOS/030_Informes/DocApoyo/040-GetListaOrdenarPorInventarios.html

Â¿CÃ³mo obtener la lista ordenada por los inventarios?

GetListaOrdenarPorInventarios (datajson, controlkey, iapp, random) : json
...
Esta función es la encargada de retornar el listado de campos que se pueden usar para ordenar la información de los siguientes informes de inventarios:
...
Retorna un Json con el listado de opciones que se puede utilizar para ordenar la informaciÃ³n de algunos reportes de inventarios.
...
| --- | --- | --- | --- |
| dataJSON | JSON | Json que contiene en su interior la siguiente estructura.  **bporcc:** Indica si el reporte de saldos de inventarios es por centro de costos o no. | { "bporcc": "T" } |
...
var URLFuncion = '/datasnap/rest/TReportes/"GetListaOrdenarPorInventarios"/';
...
Ver documentaciÃ³n de la peticiÃ³n por [GET.](PDFS/040-GetListaOrdenarPorInventarios.pdf)
...
| respuesta | JSON | Json que contiene en su interior el arreglo de objetos “datos” que se describe a continuación:  **campo (varchar):** Nombre interno del campo en la base de **datos.** **etiquete (varchar):** Nombre del campo tal y como aparece en la ventana de edición del elemento de inventario. |
