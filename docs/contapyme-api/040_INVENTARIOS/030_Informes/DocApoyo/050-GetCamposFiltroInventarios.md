...
> Fuente original: https://www.contapyme.com/api/040_INVENTARIOS/030_Informes/DocApoyo/050-GetCamposFiltroInventarios.html

Â¿CÃ³mo obtener los campos filtrados por el inventario?

GetCamposFiltroInventarios (datajson, controlkey, iapp, random) : json
...
Esta función es la encargada de retornar el listado de campos disponibles para filtrar los informes de:
...
Retorna un Json con el listado de campos que se pueden utilizar para filtrar algunos reportes de inventarios.
...
var URLFuncion = '/datasnap/rest/TReportes/"GetCamposFiltroInventarios"/';
...
Ver documentaciÃ³n de la peticiÃ³n por [GET](PDFS/050-GetCamposFiltroInventarios.pdf)
...
| respuesta | JSON | Json que contiene en su interior el arreglo “datos” que se describe a continuación:  **datos (arreglo):** contiene en cada posición el nombre del campo que se puede utilizar para realizar filtro, va el nombre del campo tal y como está en la base de datos y separado por “:” va el nombre del campo tal y como aparece en el formulario de edición del elemento de inventario. |
