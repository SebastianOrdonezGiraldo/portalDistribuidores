...
> Fuente original: https://www.contapyme.com/api/040_INVENTARIOS/010_Catalogos/010_ElemInv/150-GetListItdDescarga.html

Â¿CÃ³mo obtener la lista de descarga?
...
Retorna un Json con el listado de comportamientos que tendrÃ¡n las partes de un elemento ante las acciones que se realicen con Ã©l.
...
| --- | --- | --- | --- |
| dataJSON | JSON | Para poder obtener el listado de comportamientos que pueden tener las partes de un elemento compuesto, es necesario enviar en este parámetro el manejo del producto en el inventario y los usos de éste, para ello se deben enviar los siguientes campos:  **bcontrolinv**: Indica si al elemento se le hace control de cantidades en el inventario (requerido). **bconsumo**: Indica si el elemento es para consumo interno, ya sea para la producción o para el consumo corriente (requerido). **bventa**: Indica si el elemento estará disponible para la venta (requerido) **bproducto**: Indica si el elemento es de producción interna. (requerido) **bservicio**: Indica si el elemento es un servicio. (requerido) | { "bcontrolinv": "T", "bconsumo": "F", "bventa": "T", "bproducto": "T", "bservicio": "F" } |
...
var URLFuncion = '/datasnap/rest/TCatElemInv/"GetListItdDescarga"/';
...
Ver documentaciÃ³n de la peticiÃ³n por [GET.](PDFS/150-GetListItdDescarga.pdf)
...
{
"codigo": "E",
"descripcion": "En embodegamiento causa consumo de partes"
},
{
"codigo": "P",
"descripcion": "Combo descriptivo de partes"
}
]
}
}
...
| respuesta | JSON | Json que contiene en su interior el arreglo de objetos “datos” que se describe a continuación:  **codigo** (**varchar**): Código del comportamiento que puede tener el elemento. **descripcion** (**varchar**): Descripción del comportamiento que tendrán las partes del elemento frente a las acciones que se realicen con él. |
