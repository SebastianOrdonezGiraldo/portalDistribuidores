...
> Fuente original: https://www.contapyme.com/api/040_INVENTARIOS/010_Catalogos/010_ElemInv/080-GetExisteElemInv.html

Â¿CÃ³mo obtener el elemento existente del inventario?

GetExisteElemInv (datajson, controlkey, iapp, random) : json
...
Esta función es la encargada de validar si un elemento de inventario ya existe en la base de datos, recibe el código del elemento de inventario a validar y retorna True si el elemento existe o false si el elemento no existe en la base de datos.

#### Resultado

Retorna un Json con la confirmaciÃ³n de existencia del elemento de inventario.
...
| --- | --- | --- | --- |
| dataJSON | JSON | Json que contiene en su interior la siguiente estructura. **irecurso**: Código del elemento de inventario del cual se va a validar su existencia. (requerido) | { "irecurso": "01628" } |
...
var URLFuncion = '/datasnap/rest/TCatElemInv/"GetExisteElemInv"/';
...
Ver documentaciÃ³n de la peticiÃ³n por [GET.](PDFS/080-GetExisteElemInv.pdf)
...
| respuesta | JSON | Json que contiene en su interior el objeto “datos” que se describe a continuación.  **existe** (**varchar**): Contiene true si el elemento de inventario existe y false si no existe. |
