...
> Fuente original: https://www.contapyme.com/api/040_INVENTARIOS/010_Catalogos/010_ElemInv/090-GetValidarNuevoIdElemInv.html

Â¿CÃ³mo establecer la validaciÃ³n por ID de un nuevo elemento del inventario?

GetValidarNuevoIdElemInv (datajson, controlkey, iapp, random) : json
...
Esta función es la encargada de validar si el código (irecurso) de un nuevo elemento de inventario es correcto, valida que dicho código no contenga caracteres especiales.

#### Resultado

Retorna un Json con la confirmaciÃ³n de la validez del cÃ³digo de un nuevo elemento de inventario.
...
| --- | --- | --- | --- |
| dataJSON | JSON | Json que contiene en su interior la siguiente estructura.  **irecurso**: Identificador único del elemento de inventario. | { "irecurso": "016423" } |
...
var URLFuncion = '/datasnap/rest/TCatElemInv/"GetValidarNuevoIdElemInv"/';
...
Ver documentaciÃ³n de la peticiÃ³n por [GET.](PDFS/090-GetValidarNuevoIdElemInv.pdf)
...
| respuesta | JSON | Json que contiene en su interior el objeto “datos” que se describe a continuación.  **irecursovalido** (**varchar**): Contiene true cuando el código del nuevo elemento de inventario es válido y false cuando no lo es. |
