...
> Fuente original: https://www.contapyme.com/api/040_INVENTARIOS/010_Catalogos/030_LineasProductos/060-GetExisteLineaProductos.html

Â¿CÃ³mo obtener las lineas de productos existentes?

GetExisteLineaProductos (datajson, controlkey, iapp, random) : json
...
Esta función es la encargada de validar si una línea de productos ya existe en la base de datos, recibe el código de la línea de productos a validar y retorna True si la línea existe o false si la línea no existe en la base de datos.

#### Resultado

Retorna un Json con la confirmaciÃ³n de existencia de una lÃ­nea de productos.
...
| --- | --- | --- | --- |
| dataJSON | JSON | Json que contiene en su interior la siguiente estructura.  **idepinv**: Código del departamento o línea de productos del cual se va a validar su existencia. (requerido) | { "idepinv": "0125" } |
...
var URLFuncion = '/datasnap/rest/TCatLineasProductos/"GetExisteLineaProductos"/';
...
Ver documentaciÃ³n de la peticiÃ³n por [GET.](PDFS/060-GetExisteLineaProductos.pdf)
...
| respuesta | JSON | Json que contiene en su interior el objeto “datos” que se describe a continuación.  **existe (varchar):** Contiene true si la línea de productos existe y false si no existe. |
