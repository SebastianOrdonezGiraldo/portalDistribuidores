...
> Fuente original: https://www.contapyme.com/api/040_INVENTARIOS/010_Catalogos/030_LineasProductos/070-GetValidarNuevoIdLineaProductos.html

Â¿CÃ³mo obtener la validaciÃ³n de un nuevo ID por las lineas de productos?

GetValidarNuevoIdLineaProductos (datajson, controlkey, iapp, random) : json
...
Esta función es la encargada de validar si el código (idepinv) de una nueva línea de productos es correcto, valida que dicho código no contenga caracteres especiales.

#### Resultado

Retorna un Json con la confirmaciÃ³n de la validez del cÃ³digo de una nueva lÃ­nea de productos.
...
| --- | --- | --- | --- |
| dataJSON | JSON | Json que contiene en su interior la siguiente estructura.  **idepinv:** Identificador único del departamento o línea de productos. (requerido) | { "idepinv": "0145" } |
...
var URLFuncion = '/datasnap/rest/TCatLineasProductos/"GetValidarNuevoIdLineaProductos"/';
...
Ver documentaciÃ³n de la peticiÃ³n por [GET.](PDFS/070-GetValidarNuevoIdLineaProductos.pdf)
...
| respuesta | JSON | Json que contiene en su interior el objeto “datos” que se describe a continuación.  **idepinvvalido (varchar):** Contiene true cuando el código de la nueva línea de productos es válido y false cuando no lo es. |
