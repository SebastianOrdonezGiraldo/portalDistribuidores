...
> Fuente original: https://www.contapyme.com/api/040_INVENTARIOS/010_Catalogos/020_GruposInv/080-GetValidarNuevoIdGrupoInv.html

Â¿CÃ³mo obtener la validaciÃ³n por su ID de un nuevo grupo del inventario?

GetValidarNuevoIdGrupoInv (datajson, controlkey, iapp, random) : json
...
Esta función es la encargada de validar si el código (igrupoinv) de un nuevo grupo de inventario es correcto, valida que dicho código no contenga caracteres especiales.

#### Resultado

Retorna un Json con la confirmaciÃ³n de la validez del cÃ³digo de un nuevo grupo de inventario.
...
| --- | --- | --- | --- |
| dataJSON | JSON | Json que contiene en su interior la siguiente estructura:  **igrupoinv:** Identificador único del grupo de inventario. (requerido) | { "igrupoinv": "C01654" } |
...
var URLFuncion = '/datasnap/rest/TCatGrupoInv/"GetValidarNuevoIdGrupoInv"/';
...
Ver documentaciÃ³n de la peticiÃ³n por [GET.](PDFS/080-GetValidarNuevoIdGrupoInv.pdf)
...
| respuesta | JSON | Json que contiene en su interior el objeto “datos” que se describe a continuación.  **igrupoinvvalido (varchar):** Contiene true cuando el código del nuevo grupo de inventario es válido y false cuando no lo es. |
