...
> Fuente original: https://www.contapyme.com/api/040_INVENTARIOS/010_Catalogos/020_GruposInv/100-DoRecodificarGrupoInv.html

Â¿CÃ³mo hacer la recodificaciÃ³n de un grupo del inventario?

DoRecodificarGrupoInv (datajson, controlkey, iapp, random) : json
...
Esta función es la encargada de recodificar un grupo de inventario, es decir, cambia el código de identificación del grupo tanto en el catálogo de grupos de inventario como en las operaciones en las que esté relacionado.

#### Resultado

Retorna un Json con la confirmaciÃ³n de la recodificaciÃ³n de los datos del grupo de inventario.
...
| --- | --- | --- | --- |
| dataJSON | JSON | Json que contiene en su interior la siguiente estructura.  **igrupoinv:** Código actual del grupo de inventario a recodificar. **(requerido)** **newigrupoinv:** Nuevo código que se le asignará al grupo de inventario. (requerido) | { "igrupoinv": "C015277", "newigrupoinv": "0647551" } |
...
var URLFuncion = '/datasnap/rest/TCatGrupoInv/"DoRecodificarGrupoInv"/';
...
Ver documentaciÃ³n de la peticiÃ³n por [GET.](PDFS/100-DoRecodificarGrupoInv.pdf)
...
| respuesta | JSON | Json que contiene en su interior el objeto “datos” que se describe a continuación:  **recodificar (varchar):** contiene true cuando el grupo de inventario se recodifica satisfactoriamente y false cuando no se pudo recodificar. |
...
- 190: El grupo de inventario no existe.
- 210: El nuevo igrupoinv no es vÃ¡lido.
