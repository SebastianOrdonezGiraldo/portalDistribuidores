...
> Fuente original: https://www.contapyme.com/api/040_INVENTARIOS/010_Catalogos/020_GruposInv/070-GetExisteGrupoInv.html

Â¿CÃ³mo obtener un grupo existente del inventario?

GetExisteGrupoInv (datajson, controlkey, iapp, random) : json
...
Esta función es la encargada de validar si un grupo de inventario ya existe en la base de datos, recibe el código del grupo de inventario a validar y retorna True si el grupo existe o false si el grupo no existe en la base de datos.

#### Resultado

Retorna un Json con la confirmaciÃ³n de existencia del grupo de inventario
...
| --- | --- | --- | --- |
| dataJSON | JSON | Json que contiene en su interior la siguiente estructura:  **igrupoinv:** Código del grupo de inventario del cual se va a validar su existencia. (requerido) | { "igrupoinv": "C06124" } |
...
var URLFuncion = '/datasnap/rest/TCatGrupoInv/"GetExisteGrupoInv"/';
...
Ver documentaciÃ³n de la peticiÃ³n por [GET.](PDFS/070-GetExisteGrupoInv.pdf)
...
| respuesta | JSON | Json que contiene en su interior el objeto “datos” que se describe a continuación.  **existe (varchar):** Contiene true si el grupo de inventario existe y false si no existe. |
