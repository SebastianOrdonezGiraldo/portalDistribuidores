...
> Fuente original: https://www.contapyme.com/api/040_INVENTARIOS/025_Movimientos/050_GetListaUltPrecioCompraProducto.html

Â¿CÃ³mo obtener el precio del ultimo producto comprado?

GetListaUltPrecioCompraProducto (datajson, controlkey, iapp, random) : json
...
Esta función es la encargada de retornar el listado de los últimos precios de compra para un producto por cada bodega y proveedor.

#### Resultado

Retorna un Json con la lista de los Ãºltimos precios de compra para un producto.
...
| --- | --- | --- | --- |
| dataJSON | JSON | Este Json contiene en su interior la siguiente estructura:  **fecharef:** Fecha de referencia para la búsqueda hacia atrás del último precio. Si no se indica se asume la fecha del sistema. (Opcional).  **irecurso:** Código del producto al que se desea consultar el último precio. (requerido).  **iinventario:** Código de la bodega donde se buscará el último precio (Opcional).  **init:** Código del proveedor para buscar sólo en las compras a éste (Opcional). | { "irecurso": "75625630", "init": "890152222" } |
...
var URLFuncion = '/datasnap/rest/TInventarios/"GetListaUltPrecioCompraProducto"/';
...
Ver documentaciÃ³n de la peticiÃ³n por [GET.](PDFS/050_GetListaUltPrecioCompraProducto.pdf)
...
| respuesta | JSON | Json que contiene en su interior el arreglo de objetos “datos” los cuales serán descritos a continuación.  **datos (arreglo de objetos):** cada objeto contiene en su interior la siguiente **información:** **init:** código del proveedor enviado en la petición **(opcional)** **multprecio:** último precio al que fue comprado el producto al proveedor especificado en la petición. |
