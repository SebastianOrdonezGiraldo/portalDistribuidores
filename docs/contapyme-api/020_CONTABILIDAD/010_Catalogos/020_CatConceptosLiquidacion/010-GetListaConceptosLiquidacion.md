...
> Fuente original: https://www.contapyme.com/api/020_CONTABILIDAD/010_Catalogos/020_CatConceptosLiquidacion/010-GetListaConceptosLiquidacion.html

Â¿CÃ³mo obtener la lista de conceptos de liquidaciÃ³n?

GetListaConceptosLiquidacion (datajson, controlkey, iapp, random) : json
...
Esta función es la encargada de retornar la información de un listado de conceptos de liquidación en ingreso o egreso registrados en el sistema.   
Si se requiere obtener sólo los conceptos de liquidación en ingreso, se debe enviar en "datosfiltro":{"itdconc":"I"}, si por el contrario se requiere obtener sólo los conceptos de liquidación en egreso, se debe enviar en "datosfiltro":{"itdconc":"E"}  
Esta función retorna para cada concepto de liquidación la información que sea solicitada.

#### Resultado

Retorna un Json con el listado de conceptos de liquidaciÃ³n en ingreso o egreso registrados en el sistema.
...
| --- | --- | --- | --- |
| dataJSON | JSON | Este Json contiene en su interior la siguiente estructura:  **datospagina:** Json que contiene: **cantidadregistros:** cantidad de registros a retornar por petición (requerido) **pagina:** número de página desde la cual se retornará la información (requerido)  **camposderetorno:** Arreglo que contiene los nombres de los campos de los cuales se desea obtener la información, (requerido) Para conocer los nombres de los campos que se pueden solicitar en la petición, consultar el documento “InfoBasica” que se encuentra en la zona de “Documentación de apoyo”.  **datosfiltro:** Json que contiene la información por la cual se filtrará la búsqueda de los conceptos de liquidación.  Los campos que se pueden utilizar como filtro se encuentran en el documento “InfoBasica” de la zona de “Documentación de apoyo”.  Es muy importante tener en cuenta que si se desean obtener los conceptos de liquidación en ingreso se debe enviar: "datosfiltro":{"itdconc":"I"} y si se desean obtener los conceptos de liquidación en egreso se debe enviar: "datosfiltro":{"itdconc":"E"}  En datosfiltro también se puede enviar el parámetro “sql”, el cual contiene el fragmento Where de una consulta SQL que se desea agregar a la consulta principal, por ejemplo, si se desean obtener todos los conceptos de liquidación que en el nombre contengan la palabra “IVA” y que a su vez sean agrupadores o padres de otros conceptos, datosfiltro tendría la siguiente estructura:  **"datosfiltro":**{"sql":"nconcepto like '%IVA%' and (bagrupador = 'T')"} (opcional)  **ordenarpor:** Json que contiene la información por la cual se va a ordenar la consulta, se puede ordenar tanto de forma ascendente como descendente, los campos por los cuales se puede ordenar la consulta se encuentran en el documento “InfoBasica” de la zona de “Documentación de apoyo”. (opcional) | { "datospagina": { "cantidadregistros": "20", "pagina": "1" }, "camposderetorno": [ "iconcepto", "nconcepto" ], "datosfiltro": { "itdconc": "E" }, "ordenarpor": { "iconcepto": "asc" } } |
...
var URLFuncion = '/datasnap/rest/TCatConceptosLiquidacion/"GetListaConceptosLiquidacion"/';
...
Ver documentaciÃ³n de la peticiÃ³n por [GET.](PDFS/010-GetListaConceptosLiquidacion.pdf)
...
{
"iconcepto": "RANPPCONF",
"nconcepto": "Aportes a cajas de compensaciÃ³n familiar"
},
{
"nconcepto": "Aportes a pensiones patrono",
"iconcepto": "RANPERPAT"
},
{
